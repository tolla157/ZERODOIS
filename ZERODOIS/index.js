...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oiee @${num.split('@')[0]}\ seja bem vindo/a ao grupo *${mdata.subject}* siga as regras se não é ban!`
				...
				teks = `tchau seu gay @${num.split('@')[0]} quem sabe possa voltar`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭AGUARDE  UM POUCO BOT TRABALHANDO❬❗❭',
				success: '️❬ ✔ ❭ PRONTO SEU GAY 🖤',
				error: {
					stick: 'Eu falhei :( desculpa',
					Iv: 'digite o link direito viado☹️'
				},
				only: {
					group: '❬❗❭ COMANDO SÓ PODE SER EXECUTADO EM GRUPOS❬❗❭ ',
					ownerG: '❬❗❭ COMANDO EXCLUSIVO PARA O PIKA DO BOT.❬❗❭ ',
					ownerB: '❬❗❭  COMANDO EXCLUSIVO PARA O PIKA DO BOT.❬❗❭  ',
					admin: ' ❬ ⚠️ ❭ COMANDO PERMITIDO SOMENTE PARA MEMBROS NÃO COMUNS ❬ ⚠️ ❭ ',
					Badmin: '❬❗❭O BOT PRECISA SER ADEMIRO ;3 '
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5514998729679@s.whatsapp.net"] 
...
case 'lofi':
                ...
                case 'reportarbug':
                    ...
                    client.sendMessage('5514998729679@s.whatsapp.net', options, text, {quoted: mek})
           ...
           case 'rebaixar':
			...
			case 'remover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`removi um gay @${mentioned[0].split('@')[0]} fodassi🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
